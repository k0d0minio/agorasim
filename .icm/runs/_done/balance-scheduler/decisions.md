...
- D-7 — `CRON_SECRET` in `web/.env.example` carries `# [production,preview]`: `env.sh audit --changed` flagged it (the dispatcher route that reads it changed) as missing from Vercel development, but the cron runs only on Vercel and development generates its own locally, per the key's note. Build.
