...
1. **Schema and migration** — `web/src/db/schema.ts` (`messageKindEnum` gains `quote-refunded`
   on the end), a new `web/drizzle/0029_*.sql` (`ALTER TYPE message_kind ADD VALUE`), and
   whatever the once-per-refund message key needs. The existing `message_log_quote_receipt_key`
   is (kind, recipient, quote), which allows only one refund email per quote. Key the refund
   email on the Stripe refund id (a nullable `subject_ref` column plus a partial unique index,
   or an equivalent), and never widen the receipt index for the other kinds. Load the
...
5. **Email** — `web/src/content/emails.ts` gains the PT/EN `quote-refunded` template (quote
   ...
   `lib/message-log.ts` sends it once per Stripe refund id. It is called from the reconciler
