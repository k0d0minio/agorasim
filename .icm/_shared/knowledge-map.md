...
under `.icm/docs/`. Three pages of product knowledge live **outside** that tree and are named in
prose (the validator would otherwise look for them under `.icm/docs/`): the facts of record in
workspaces/_config/business-facts.md, the admin's design rules in web/docs/admin-mobile-design-spec.md,
and the client's phone guide in web/docs/guia-telemovel.md. They are pages all the same — Release
keeps them current, the knowledge lane may edit them.
...
### Product facts, design rules, the team's guide — outside the tree
...
- web/docs/admin-mobile-design-spec.md — the admin's HIG-informed mobile-first rules: Rita's
  phone is the reference device, nothing below 12px, ≥44px targets, Portuguese only (D4).
- web/docs/guia-telemovel.md — the install-and-first-week guide Diogo & Rita follow on their
  phones. An admin change that alters what they see updates this page in the same PR.
...
| **Define**                | the register's Business logic and Decisions (the `D-n` a spec must honour); `.icm/docs/data-protection.md` when the change touches personal data or a processor; `.icm/docs/admin-pt-inventory.md` for any admin vocabulary; the admin design spec for any admin UI; `/AGENTS.md` § Where do I go for `touches:`                          | —                                                                                                                                                                                  |
| **Build**                 | the code rules (`.icm/_shared/conventions.md` → `/AGENTS.md` § Conventions and web/AGENTS.md); the admin design spec for admin UI; business-facts for any fact that renders                                                                                                                                                              | —                                                                                                                                                                                  |
| **Release**               | the pages a shipped change makes stale: `.icm/docs/data-protection.md` (a processor or a data flow), business-facts (a fact), the phone guide (an admin screen), `.icm/docs/launch-runbook.md` (an ops step)                                                                                                                              | **updates** the affected page(s) in the same feature PR. No changelog in this repo (`.icm/_shared/project-rules.md` → Announcing)                                                          |
