...
- **The environments' databases** — Neon project `nameless-sea-98952497` (Kodominio, a
  ...
  branch, **not yet protected** (an act owed). Previews are to be the Vercel integration's
  `preview/<git-branch>` (`neon.previews: vercel`) — **the integration's Preview-branching
  toggle is off today and is the first act owed**: the branch list holds `main` alone, so every
  preview built so far, and the `uat` branch until the toggle is on, reads the Preview
  environment's `DATABASE_URL`, which can only point at production's branch. The UAT branch's
  database will be `preview/uat`, created by the integration on the branch's first deployment.
  Migrations reach previews and UAT at build only once the build runs the migrate step — **an
  act owed, Jamie's choice** between the Vercel build command and a `vercel-build` script
  (`pnpm db:migrate && pnpm db:verify && next build` is the candidate; `db-migrate.yml` keeps
  applying production's on `main`). `.github/workflows/neon-cleanup.yaml` (seeded 2026-09-23)
  deletes a PR's `preview/*` and `run/*` branches on close and needs `NEON_API_KEY` as an
  Actions secret (an act owed). The stray branch `verify-0026-quote-flow-message-kinds` (a
  console verification copy from 2026-09-18) is not the pipeline's — Jamie's to delete.
  `db-env.sh init` lists the acts; `db-env.sh status` reads the project once the key is exported;
  `db-env.sh reset-uat --apply` is the operator's reset after a promotion.
