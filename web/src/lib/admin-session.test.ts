...
describe("createSessionToken / verifySessionToken", () => {
  ...
  it("carries the account, a session id and an issued-at, and expires in 7 days", async () => {
    const before = Math.floor(Date.now() / 1000);
    const session = await readSessionToken(await createSessionToken(USER_ID));
  ...
  it("rejects a token that names a different account under the same signature", async () => {
    const token = await createSessionToken(USER_ID);
    const [encoded, signature] = token.split(".");

    const payload = JSON.parse(Buffer.from(encoded!, "base64url").toString("utf8"));
    const swapped = `${encodePayload({ ...payload, uid: "someone-else" })}.${signature}`;

    expect(await readSessionToken(swapped)).toBeNull();
  });
  ...
  it("rejects a v1 token, which named no account", async () => {
    // The pre-accounts format. Correctly signed tokens of this shape cannot be
    // mapped onto a user, so the version bump forces a re-login.
    const encoded = encodePayload({ v: 1, sid: "old", iat: 0, exp: 4102444800 });
  ...
  it("rejects a well-formed payload that is missing fields, even correctly signed", async () => {
    const encoded = encodePayload({ v: 2, iat: 0, exp: 4102444800 });
    expect(await readSessionToken(`${encoded}.whatever`)).toBeNull();
  });
});
